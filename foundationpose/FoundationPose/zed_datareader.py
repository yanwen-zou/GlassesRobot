...
class YcbineoatReader:
  def __init__(self, video_dir, downscale=1, shorter_side=None, zfar=np.inf):
      ...
      self.color_files = sorted(
          ...
          + glob.glob(f"{self.video_dir}/rgb/*.jpg")
          + glob.glob(f"{self.video_dir}/rgb/*.jpeg")
      )

      if len(self.color_files) == 0:
          raise FileNotFoundError(f"No color images found in {self.video_dir}/rgb (looking for .png or .jpg)")

      self.K = np.loadtxt(f'{video_dir}/cam_K.txt').reshape(3, 3)

      self.id_strs = [os.path.splitext(os.path.basename(cf))[0] for cf in self.color_files]

      first_frame = cv2.imread(self.color_files[0])
      if first_frame is None:
          raise FileNotFoundError(f"无法读取图像: {self.color_files[0]}")
