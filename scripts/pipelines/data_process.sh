...
BALL_PIPELINE="${PROJECT_ROOT}/scripts/calib/balls/run_ball_pipeline.sh"
DEFAULT_DATA_ROOT="${PROJECT_ROOT}/data"
DATA_ROOT="$DEFAULT_DATA_ROOT"
SCALE=0.75
ZED_INTR_READER="${PROJECT_ROOT}/scripts/data_processing/zed_intr_reader.py"
HAND_MASK_SCRIPT="${PROJECT_ROOT}/scripts/data_processing/grounded_sam_hand_masks.py"
...
if [ "${#BALL_PIPELINE_EPISODES[@]}" -gt 0 ]; then
  ...
  #     "${PROJECT_ROOT}/scripts/calib/balls/compute_aligned_cam_pose.py" \
  #     --episode_dir "${DATA_ROOT}/${episode}"
  # done
else
