...
def main():
    """Initialize i2rt via RPC and run a small joint1 oscillation loop."""
    ...
    try:
        ...
        print("RPC initialized. Press Ctrl+C to stop.")
        while True:
            for sign in (1.0, -1.0):
                target_q = base_q.copy()
                target_q[0] = base_q[0] + np.deg2rad(sign * joint1_amp_deg)
                client.send_joint_pos_rad(target_q, duration=move_duration, steps=move_steps)
