...
Example:
    python src/egodata_eval/vis_pointcloud_sequence.py \
        --data_path data/moving/20251125_210453 --fps 5
    
    # Transform point clouds from camera to base coordinates no need of head pose
    python src/egodata_eval/vis_pointcloud_sequence.py \
    --data_path /home/akihi/code/GlassesRobot/data/train/20251125_210453
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data_path", type=str, default="data/20251125_210453", help="Sequence directory path.")
...
def main():
    ...
    if not cam_to_base_path.exists():
        raise FileNotFoundError(f"cam_to_base.npy not found in {seq_path}; cannot transform to base coordinates.")
    ...
    for fid, cloud in frame_clouds:
        ...
        if T_base_cam is None:
            print(f"[WARN] No cam_to_base transform for frame {fid}, skipping this frame")
            continue
        points = cloud[:, :3].astype(np.float32)
        if points.size == 0:
            print(f"[WARN] Empty point cloud for frame {fid}, skipping")
            continue
        ones = np.ones((points.shape[0], 1), dtype=np.float32)
        homo = np.concatenate([points, ones], axis=1)
    ...
    if not frame_clouds_proc:
        raise RuntimeError("No frames to display after transforming to base coordinates.")
    ...
    if ball_centers_path.exists():
        try:
            ...
            for fid, centers in raw_ball_centers.items():
                T_base_cam = cam_to_base.get(fid)
                if T_base_cam is None:
                    print(f"[WARN] No cam_to_base transform for ball centers frame {fid}, skipping")
                    continue
