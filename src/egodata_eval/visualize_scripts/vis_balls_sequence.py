...
def main():
    ...
    mask_dir = args.mask_dir or (episode_dir / "mask_balls")
    ...
    for frame_idx, fid in enumerate(iter_frames(headposes)):
        ...
        translation = camref_from_cam[:3, 3].astype(np.float32)
        rotation = camref_from_cam[:3, :3].astype(np.float32)
        ...
        # Full-frame cloud aligned to the first frame using head poses.
        full_pts, full_cols = backproject_full_depth(depth_m, intr, camref_from_cam, rgb)
        if full_pts.size > 0:
