...
Example:
    python src/egodata_eval/vis_multi_pointcloud.py \
        --data_path data/seq1 data/seq2 --fps 5
"""
...
def _rotmat_to_rpy(R: np.ndarray) -> Tuple[float, float, float]:
    """Convert rotation matrix to roll-pitch-yaw (XYZ intrinsic), in radians."""
        ...
        if ball_centers is not None:
            ...
            if frame_balls is not None and len(frame_balls) > 0:
                ...
                for ball_id in sorted(frame_balls.keys()):
                    pos = frame_balls[ball_id]  # Already in target coordinate system
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="One or more sequence directory paths; each must contain cam_to_base.npy.",
    )
...
def main():
    ...
    all_frame_clouds: List[Tuple[str, np.ndarray]] = []
    all_camera_poses: List[Tuple[str, np.ndarray]] = []
    all_anchor_frames: List[Tuple[str, np.ndarray]] = []
    all_ball_centers: Dict[str, Dict[int, np.ndarray]] = {}
    total_frames = 0

    for seq_path_str in args.data_path:
        seq_path = Path(seq_path_str)
        if not seq_path.exists():
            print(f"[WARN] Skip {seq_path}: path does not exist")
            continue
        ...
        if not frame_clouds:
            print(f"[WARN] No frames collected for sequence {seq_id}, skipping.")
            continue

        cam_to_base_path = seq_path / "cam_to_base.npy"
        if not cam_to_base_path.exists():
            print(f"[WARN] cam_to_base.npy not found in {seq_path}; skipping this sequence.")
            continue
        ...
        if frame_ids_arr is None or transforms_arr is None:
            print(f"[WARN] cam_to_base.npy at {cam_to_base_path} missing expected keys, skipping {seq_id}")
            continue
        if frame_ids_arr.shape[0] != transforms_arr.shape[0]:
            print(f"[WARN] cam_to_base.npy frame/transform mismatch for {seq_id}, skipping.")
            continue
        cam_to_base: dict[str, np.ndarray] = {
        ...
        print(f"[INFO] Transforming point clouds for {seq_id} to base coordinate system")
        ...
        for fid, cloud in frame_clouds:
            ...
            if T_base_cam is None:
                print(f"[WARN] No cam_to_base transform for frame {fid} in {seq_id}, skipping this frame")
                continue
            points = cloud[:, :3].astype(np.float32)
            if points.size == 0:
                print(f"[WARN] Empty point cloud for frame {fid} in {seq_id}, skipping")
                continue
            ones = np.ones((points.shape[0], 1), dtype=np.float32)
            homo = np.concatenate([points, ones], axis=1)
        ...
        if not frame_clouds_proc:
            print(f"[WARN] No frames to display for {seq_id} after transforming to base; skipping.")
            continue

        anchor_frames: List[Tuple[str, np.ndarray]] = [(f"{seq_id}_base_frame", np.eye(4, dtype=np.float32))]

        # Load and transform ball centers if available
        ball_centers_data = None
        ball_centers_path = seq_path / "ball_centers.txt"
        if ball_centers_path.exists():
            try:
                raw_ball_centers = load_ball_centers(ball_centers_path)
                ball_centers_data = {}
                for fid, centers in raw_ball_centers.items():
                    T_base_cam = cam_to_base.get(fid)
                    if T_base_cam is None:
                        print(f"[WARN] No cam_to_base transform for ball centers frame {fid} in {seq_id}, skipping")
                        continue
                    transformed: Dict[int, np.ndarray] = {}
                    for bid, pos in centers.items():
                        pos_homo = np.array([pos[0], pos[1], pos[2], 1.0], dtype=np.float32)
                        pos_base = (T_base_cam @ pos_homo)[:3]
                        transformed[bid] = pos_base.astype(np.float32)
                    if transformed:
                        ball_centers_data[fid] = transformed
                print(f"[INFO] Loaded and transformed ball centers from {ball_centers_path} ({len(ball_centers_data)} frames)")
            except Exception as e:
                print(f"[WARN] Failed to load ball centers from {ball_centers_path}: {e}")

        # Re-key frames to make playback continuous and unique
        for fid, cloud in frame_clouds_proc:
            new_id = f"{seq_id}_{fid}"
            all_frame_clouds.append((new_id, cloud))
        for fid, pose in camera_poses:
            new_id = f"{seq_id}_{fid}"
            all_camera_poses.append((new_id, pose))
        if ball_centers_data:
            for fid, centers in ball_centers_data.items():
                new_id = f"{seq_id}_{fid}"
                all_ball_centers[new_id] = centers
        all_anchor_frames.extend(anchor_frames)
        total_frames += len(frame_clouds_proc)
        print(f"[INFO] Prepared {len(frame_clouds_proc)} frames for {seq_id}")

    if not all_frame_clouds:
        raise RuntimeError("No valid sequences to visualize after processing all data_path entries.")

    print(f"[INFO] Visualizing {total_frames} frames across {len(args.data_path)} sequences.")
    
    visualize_sequence(
        frame_clouds=all_frame_clouds,
        seq_id="multi",
        fps=args.fps,
        point_radius=args.point_radius,
        spawn_viewer=not args.no_spawn,
        camera_poses=all_camera_poses,
        anchor_frames=all_anchor_frames,
        ball_centers=all_ball_centers if all_ball_centers else None,
    )

    # Plot T_base_cam translation and rotation over frames
    pose_map = {fid: pose for fid, pose in all_camera_poses}
    trans_list: List[np.ndarray] = []
    rpy_list: List[Tuple[float, float, float]] = []
    frame_indices: List[int] = []
    for idx, (fid, _) in enumerate(all_frame_clouds):
        pose = pose_map.get(fid)
        if pose is None:
            continue
        trans_list.append(pose[:3, 3])
        rpy_list.append(_rotmat_to_rpy(pose[:3, :3]))
        frame_indices.append(idx)

    if trans_list:
        trans_arr = np.vstack(trans_list)
        rpy_arr = np.vstack(rpy_list)
        plt.figure(figsize=(10, 4))
        plt.title("T_base_cam translation (meters)")
        plt.plot(frame_indices, trans_arr[:, 0], label="tx")
        plt.plot(frame_indices, trans_arr[:, 1], label="ty")
        plt.plot(frame_indices, trans_arr[:, 2], label="tz")
        plt.xlabel("frame index (combined)")
        plt.ylabel("translation (m)")
        plt.legend()
        plt.grid(True, linestyle="--", alpha=0.4)

        plt.figure(figsize=(10, 4))
        plt.title("T_base_cam rotation (roll/pitch/yaw, degrees)")
        plt.plot(frame_indices, np.degrees(rpy_arr[:, 0]), label="roll")
        plt.plot(frame_indices, np.degrees(rpy_arr[:, 1]), label="pitch")
        plt.plot(frame_indices, np.degrees(rpy_arr[:, 2]), label="yaw")
        plt.xlabel("frame index (combined)")
